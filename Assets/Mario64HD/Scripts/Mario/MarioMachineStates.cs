...
public partial class MarioMachine : SuperStateMachine {
    ...
    void Idle_SuperUpdate()
    {
		...
		if (IsSliding() && moveDirection == Vector3.zero && starred == false)
        {
		...
		if (!isTakingFallDamage && input.Current.StrikeDown && starred == false)
        {
            currentState = MarioStates.Strike;
            return;
        }

		if (!isTakingFallDamage && input.Current.Trigger && starred == false)
        {
            currentState = MarioStates.Crouch;
            return;
        }

		if (input.Current.MoveInput != Vector3.zero && starred == false)
        {
