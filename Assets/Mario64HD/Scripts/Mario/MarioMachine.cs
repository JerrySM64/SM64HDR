...
public partial class MarioMachine : SuperStateMachine
{
    ...
    void Start()
    {
		...
		public void OnTriggerEnter(Collider other){
				...
				if (other.gameObject.CompareTag ("stopTimerZone")) {
					GameObject.FindObjectOfType<GameMaster> ().stopTimer = true;
				}

				if (other.gameObject.CompareTag ("BobOmbEnter")) {
					GameObject.FindObjectOfType<GameMaster> ().bobOmbEnter = true;
					GameObject.FindObjectOfType<GameMaster> ().pausable = false;
				}
		       
		        if (other.gameObject.CompareTag ("DoorEnter")) {
		         	GameObject.FindObjectOfType<GameMaster> ().enterCastle = true;
			        GameObject.FindObjectOfType<GameMaster> ().pausable = false;
		        }

				if (other.gameObject.CompareTag ("WhompsEnter")) {
					GameObject.FindObjectOfType<GameMaster> ().whompsEnter = true;
					GameObject.FindObjectOfType<GameMaster> ().pausable = false;
				}
        if (other.gameObject.CompareTag ("TowerTrigger")) {
            StartCoroutine(theTowerScript());
        }

        if (other.gameObject.CompareTag("easterEggCastle"))
        {
            GameObject.FindObjectOfType<SimpleFlips>().GBJ();
        }

        if (other.gameObject.CompareTag ("backOut")) {
			GameObject.FindObjectOfType<GameMaster> ().exitCastle = true;
			GameObject.FindObjectOfType<GameMaster> ().pausable = false;
		}

		if (other.gameObject.CompareTag ("squished")) {
			HeavyDamage (4, new Vector3(100,100,100));
            //GroundDamageLight (4, new Vector3(100,100,100));
            //isTakingFallDamage = true;
            //ScaleAnimator.GetComponent<Animation>().Play();
        }

		if (other.gameObject.CompareTag ("whompSquish")) {
			HeavyDamage (3, new Vector3(100,100,100));
			//getSquished (0.9);
			//GroundDamageLight (4, new Vector3(100,100,100));
			//isTakingFallDamage = true;
			//ScaleAnimator.GetComponent<Animation>().Play();
		...
		if (other.gameObject.CompareTag ("antiUpwarp")) {
			GameObject.FindObjectOfType<SuperCharacterController> ().antiUpwarp();
		}

    }

	public void getSquished(double y){
		if (y > 1)
			transform.localScale -= new Vector3 (0, 0.99f, 0);
		else
			transform.localScale -= new Vector3 (0, (float)y, 0);
	}
	...
	public void checkPound (){
		if (StateCompare (MarioStates.GroundPound)) {
			pounding = true;
		} else {
			pounding = false;
		}
    ...
    IEnumerator theTowerScript()
    {
        GameObject.FindObjectOfType<MovingPlatformTower>().marioOn = true;
        yield return new WaitForSeconds(6.5f);
        GameObject.FindObjectOfType<MovingPlatformTower>().marioOn = false;
        yield return new WaitForSeconds(3f);
        GameObject.FindObjectOfType<MovingPlatformTower>().reached = true;
    }
}
